...
(define (domain CoachingPotatoPlanner)
    ...
    (:action sin-preparador
        :parameters (?e - ejercicio ?d - dia ?n - nivel ?l - nivel)
        :precondition (and  (not (exists (?x - ejercicio) (preparador ?x ?e)))
    ...
    (:action con-preparador
        :parameters (?e - ejercicio ?d - dia ?n - nivel ?l - nivel)
        :precondition (and  (exists (?x - ejercicio) (and (preparador ?x ?e) (asig ?x ?d)))
                            (imply (not (primer_dia ?d)) (exists (?ayer - dia) (and (ant ?ayer ?d) (asig ?e ?ayer))))
                            (dificultad ?n ?e)
                            (not (objetivo ?n ?e))
                            (sig ?n ?l)
